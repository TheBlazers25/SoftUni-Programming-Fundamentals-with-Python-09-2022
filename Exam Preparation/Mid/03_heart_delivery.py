...
while True:
    ...
    cupid_posistion += int(command.split()[-1])

    if cupid_posistion >= len(neighborhood):
        cupid_posistion = 0

    if split_command[0] == 'Jump':
        neighborhood[cupid_posistion] -= 2
        if neighborhood[cupid_posistion] == 0:
            print(f"Place {cupid_posistion} has Valentine's day.")
        else:
            if neighborhood[cupid_posistion] == 0:
                print(f"Place {cupid_posistion} had Valentine's day.")

print(f"Cupid's last position was {cupid_posistion}.")
